...
==> 급여정도에 대한 부분을 비 관련자로부터 차단을 할 수가 있다.
...
2. SELECT *
   FROM v_emp; 결과가 몇 건 일가?   --view는 실체가없이 
                                  --데이터 집합을 정의하는 sql 그 자체라서 16개가 조회될거야 
                                  --해당 SQL에서 사용하는 테이블의 데이터가 변경이 되면
                                  --view에도 영향을 미친다


view는 SQL 이기때문에 조인된 결과나, 그룹함수를 적용하여 
...
SELECT dname, emp.deptno, ename, job, hiredate
...
SEQUENCE : 종복되지 않는 정수값을 반환해주는 오라클 객체
시작값 : (default 1, 혹은 개발자가 설정 가능) 부터 순차적으로 증가한 값을 반환한다.
...
emp 테이블에서 empno = 7698 인 데이터를 조회한다.
...
---------------------------------------------------
   ...
   2 - access("EMPNO"=7698)   -- 이 위치로 빠르게 접근하였다.


emp 테이블에 primary key 제약조건을 생성하고나서 변경된 점
 * 오라클 입장에서 데이터를 조회할 떄 사용할 수 있는 전략이 하나 더 생김
 1. table full scan 
...
|*  1 |  INDEX UNIQUE SCAN| PK_EMP |     1 |     4 |     0   (0)| 00:00:01 |     --테이블까지 접근을 하지않음. 
----------------------------------------------------------------------------
...
empno 컬럼의 인덱스를 unique 인덱스가 아닌 일반 인덱스(중복이 가능한)로 생상한 경우
...
1. non Unique 인덱스 생성
   ...
   non_uni        : IDX_N_테이블명_02;
   U
CREATE [UNIQUE]INDEX 인 덱스명 ON 테이블 (인덱스로 구성할 컬럼);
...
|*  2 |   INDEX RANGE SCAN          | IDX_NU_EMP_01 |     1 |       |     1   (0)| 00:00:01 |    -- UNIQUE -> RANGE
---------------------------------------------------------------------------------------------
