...
SELECT t, LEVEL, LTRIM(SYS_CONNECT_BY_PATH(t,'-'),'-') tree
FROM imsi
CONNECT BY LEVEL <= 3;
...
(단 급여가 같을경우 입사일자가 빠른 사람이 우선순위가 높다);
...
WINDOWING : 파티션 내의 행동을 세부적으로 선별하는 범위 기술
...
WINDOWING : 기본 설정값 존재 : RANGE UNBOUNDED PRECEDING
...
SELECT empno, ename, deptno, sal, 
       SUM(sal) OVER(PARTITION BY deptno ORDER BY sal, deptno ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) c_sum
...
SELECT empno, ename, sal, (SELECT MAX(SUM)
                           FROM emp
                           WHERE 
FROM emp;
...
논리 모델 : 시스템에서 필요로하는 엔터티(테이블), 엔터티의 속성, 엔터티간의 관계
           데이터가 어떻게 저장될지는 관심대상이 아니다 => 물리모델에서 고려할 사항
           논리모델에서는 데이터의 전반적인 큰 틀을 설계
...
관계차수: 1-N,  1-1,   N-N ==> 1:N으로 변경할 대상
         수직바, 까마귀발
관계옵션: mandatory(필수), optional(옵션) o표기




요구사항(요구사항 기술사, 장표, 인터뷰)에서 명사 ==> 엔터티 or 속성일 확률이 높음
